...
//                let started = f[FolderTable.started]
    ...
    func Folder_GetWordLists(f: Folder) -> [WordList] {
        ...
        do {
            for W in try db.prepare(table) {
                ...
                wordList.dir = Path_WordList(f: f, W: wordList)
    ...
    /// get all WordLists in a Folder table
    func WordList_GetWords(f: Folder, W: WordList) -> [Word] {
        switch W.language {
        case .english:
    ...
    private func P_WordList_GetWords<T: Word>(f: Folder, W: WordList, wordType: T.Type) -> [any Word] {
        let table = Table(Name_WordList(f: f, W: W))
        do {
            let words = T.WordList_GetWords(rows: try db.prepare(table), WordListDir: Path_WordList(f: f, W: W))
            return words
        } catch {
            fatalError("P_WordList_GetWords: \(error.localizedDescription)")
        }
    }
    ...
    ///     - True: the record is added successfully
    ///     - False: the record may have duplicates in the database
    func WordList_AddWord(f: Folder, W: WordList, w: any Word) -> Bool {
        let table = Table(Name_WordList(f: f, W: W))
            ...
            for (index, image) in images.enumerated() {
                if let resizedImage = image.resized(toWidth: 300), // Resize to desired width
                   let imageData = resizedImage.jpegData(compressionQuality: 0.8) { // Convert to JPEG with compression quality
                    let imageDestinationURL = URL(fileURLWithPath: "\(wordDirPath)/image\(index).jpg")
    ...
    func WordList_DeleteWord(f: Folder, W: WordList, w: any Word) {
        let table = Table(Name_WordList(f: f, W: W))
        w.WordList_DeleteWord(db: db, table: table)
    }
    ...
    func WordList_EditWord(f: Folder, W: WordList, w: any Word) {
        let table = Table(Name_WordList(f: f, W: W))
        w.WordList_EditWord(db: db, table: table)
    }
}
